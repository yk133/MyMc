...
public final class WorldEntitySpawner
{
                                ...
                                for (int k2 = 0; k2 < 3; ++k2)
                                {
                                    ...
                                    for (int i4 = 0; i4 < l3; ++i4)
                                    {
                                        ...
                                        if (!worldServerIn.isAnyPlayerWithinRangeAt((double)f, (double)i3, (double)f1, 24.0D) && blockpos1.distanceSq((double)f, (double)i3, (double)f1) >= 576.0D)
                                        {
                                            ...
                                            if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype, biome$spawnlistentry, blockpos$mutableblockpos) && func_180267_a(EntitySpawnPlacementRegistry.func_180109_a(biome$spawnlistentry.field_76300_b), worldServerIn, blockpos$mutableblockpos))
                                            {
                                                ...
                                                if (canSpawn == net.minecraftforge.fml.common.eventhandler.Event.Result.ALLOW || (canSpawn == net.minecraftforge.fml.common.eventhandler.Event.Result.DEFAULT && (entityliving.getCanSpawnHere() && entityliving.isNotColliding())))
                                                {
                                                    ...
                                                    ientitylivingdata = entityliving.func_180482_a(worldServerIn.getDifficultyForLocation(new BlockPos(entityliving)), ientitylivingdata);
    ...
    {
        if (p_185331_0_.isBlockNormalCube())
        {
            return false;
        }
    ...
    {
        {
            ...
            {
                return iblockstate.getMaterial() == Material.WATER && p_180267_1_.getBlockState(p_180267_2_.down()).getMaterial() == Material.WATER && !p_180267_1_.getBlockState(p_180267_2_.up()).isNormalCube();
            }
            else
            {
                BlockPos blockpos = p_180267_2_.down();
                IBlockState state = p_180267_1_.getBlockState(blockpos);

                if (!state.getBlock().canCreatureSpawn(state, p_180267_1_, blockpos, p_180267_0_))
                {
                    return false;
                }
                else
