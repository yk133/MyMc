...
public class RenderGlobal implements IWorldEventListener, IResourceManagerReloadListener
{
        ...
        if (!flag && this.displayListEntitiesDirty)
        {
            ...
            else
            {
                ...
                for (int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j)
                {
                    for (int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k)
                    {
                        ...
                        if (renderchunk1 != null && p_174970_4_.isBoundingBoxInFrustum(renderchunk1.boundingBox.expand(0.0, blockpos1.getY() > 0 ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY, 0.0))) // Forge: fix MC-73139
                        {
            ...
            while (!queue.isEmpty())
            {
                ...
                for (EnumFacing enumfacing1 : EnumFacing.values())
                {
                    ...
                    if ((!flag1 || !renderglobal$containerlocalrenderinformation1.hasDirection(enumfacing1.getOpposite())) && (!flag1 || enumfacing2 == null || renderchunk3.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing1)) && renderchunk2 != null && renderchunk2.setFrameIndex(p_174970_5_) && p_174970_4_.isBoundingBoxInFrustum(renderchunk2.boundingBox))
                    {
    ...
    public int func_174977_a(BlockRenderLayer p_174977_1_, double p_174977_2_, int p_174977_4_, Entity p_174977_5_)
    {
        ...
        if (p_174977_1_ == BlockRenderLayer.TRANSLUCENT)
        {
            ...
            if (d0 * d0 + d1 * d1 + d2 * d2 > 1.0D)
            {
                ...
                for (RenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfos)
                {
                    if (renderglobal$containerlocalrenderinformation.renderChunk.compiledChunk.isLayerStarted(p_174977_1_) && k++ < 15)
                    {
        ...
        for (int j = i1; j != i; j += j1)
        {
            ...
            if (!renderchunk.getCompiledChunk().isLayerEmpty(p_174977_1_))
            {
                ++l;
                this.renderContainer.addRenderChunk(renderchunk, p_174977_1_);
            }
        }
        ...
        this.renderBlockLayer(p_174977_1_);
        this.mc.profiler.endSection();
        return l;
    }
    ...
    {
        net.minecraftforge.client.IRenderHandler renderer = this.world.dimension.getSkyRenderer();
        if (renderer != null)
        {
            renderer.render(p_174976_1_, world, mc);
            return;
        }
        ...
        else if (this.mc.world.dimension.isSurfaceWorld())
        {
            ...
            Vec3d vec3d = this.world.getSkyColor(this.mc.getRenderViewEntity(), p_174976_1_);
            ...
            if (afloat != null)
            {
                ...
                GlStateManager.pushMatrix();
                GlStateManager.rotatef(90.0F, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotatef(MathHelper.sin(this.world.getCelestialAngleRadians(p_174976_1_)) < 0.0F ? 180.0F : 0.0F, 0.0F, 0.0F, 1.0F);
                GlStateManager.rotatef(90.0F, 0.0F, 0.0F, 1.0F);
                float f6 = afloat[0];
                float f7 = afloat[1];
                float f8 = afloat[2];
            ...
            GlStateManager.rotatef(this.world.getCelestialAngle(p_174976_1_) * 360.0F, 1.0F, 0.0F, 0.0F);
            float f17 = 30.0F;
            ...
            tessellator.draw();
            GlStateManager.disableTexture2D();
            float f15 = this.world.getStarBrightness(p_174976_1_) * f16;
            ...
            double d3 = this.mc.player.getEyePosition(p_174976_1_).y - this.world.getHorizon();
    ...
    {
        if (net.minecraftforge.fml.client.FMLClientHandler.instance().renderClouds(this.ticks, p_180447_1_)) return;
        if (this.mc.world.dimension.isSurfaceWorld())
        {
            ...
            else
            {
                ...
                GlStateManager.enableBlend();
                GlStateManager.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
                Vec3d vec3d = this.world.getCloudColour(p_180447_1_);
                float f = (float)vec3d.x;
                float f1 = (float)vec3d.y;
                float f2 = (float)vec3d.z;
    ...
    private void func_180445_c(float p_180445_1_, int p_180445_2_, double p_180445_3_, double p_180445_5_, double p_180445_7_)
    {
        ...
        GlStateManager.enableBlend();
        GlStateManager.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
        Vec3d vec3d = this.world.getCloudColour(p_180445_1_);
    ...
    public void renderWorldBorder(Entity entityIn, float partialTicks)
    {
        ...
        if (entityIn.posX >= worldborder.maxX() - d3 || entityIn.posX <= worldborder.minX() + d3 || entityIn.posZ >= worldborder.maxZ() - d3 || entityIn.posZ <= worldborder.minZ() + d3)
        {
            ...
            GlStateManager.color4f(f, f1, f2, (float)d4);
    ...
    public void drawSelectionBox(EntityPlayer player, RayTraceResult movingObjectPositionIn, int execute, float partialTicks)
    {
        if (execute == 0 && movingObjectPositionIn.type == RayTraceResult.Type.BLOCK)
        {
            ...
            if (iblockstate.getMaterial() != Material.AIR && this.world.getWorldBorder().contains(blockpos))
            {
                ...
                drawSelectionBoundingBox(iblockstate.func_185918_c(this.world, blockpos).grow(0.0020000000949949026D).offset(-d3, -d4, -d5), 0.0F, 0.0F, 0.0F, 0.4F);
            }
    ...
    private Particle func_190571_b(int p_190571_1_, boolean p_190571_2_, boolean p_190571_3_, double p_190571_4_, double p_190571_6_, double p_190571_8_, double p_190571_10_, double p_190571_12_, double p_190571_14_, int... p_190571_16_)
    {
        ...
        {
            int k1 = this.calculateParticleLevel(p_190571_3_);
    ...
    public void playEvent(EntityPlayer player, int type, BlockPos blockPosIn, int data)
    {
        ...
        switch (type)
        {
            ...
            case 1031:
                ...
                break;
            case 1032:
                this.mc.getSoundHandler().play(PositionedSoundRecord.getMasterRecord(SoundEvents.BLOCK_PORTAL_TRAVEL, random.nextFloat() * 0.4F + 0.8F));
                break;
            case 1033:
            ...
            case 2001:
                ...
                {
                    SoundType soundtype = block.getSoundType(Block.func_176220_d(data), world, blockPosIn, null);
