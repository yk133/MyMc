...
public class ItemRenderer
{
    ...
    private void renderMapFirstPerson(ItemStack stack)
    {
        ...
        tessellator.draw();
        MapData mapdata = ((net.minecraft.item.ItemMap) stack.getItem()).func_77873_a(stack, this.mc.world);
        ...
        else
        {
            ...
            if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == hand)
            {
                ...
                switch (stack.getUseAction())
                {
                    ...
                    case BOW:
                        this.transformSideFirstPerson(enumhandside, equippedProgress);
                        GlStateManager.translatef((float)j * -0.2785682F, 0.18344387F, 0.15731531F);
                        GlStateManager.rotatef(-13.935F, 1.0F, 0.0F, 0.0F);
                        GlStateManager.rotatef((float)j * 35.3F, 0.0F, 1.0F, 0.0F);
    ...
    private void renderFireInFirstPerson()
    {
        ...
        {
            GlStateManager.pushMatrix();
            TextureAtlasSprite textureatlassprite = this.mc.getTextureMap().getAtlasSprite("minecraft:blocks/fire_layer_1");
            this.mc.getTextureManager().bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
