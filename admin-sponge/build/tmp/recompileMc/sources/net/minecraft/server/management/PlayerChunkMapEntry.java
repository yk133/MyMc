...
public class PlayerChunkMapEntry
{
    ...
    private Runnable loadedRunnable = new Runnable()
    {
        public void run()
        {
            PlayerChunkMapEntry.this.chunk = PlayerChunkMapEntry.this.playerChunkMap.getWorld().getChunkProvider().func_186028_c(PlayerChunkMapEntry.this.pos.x, PlayerChunkMapEntry.this.pos.z);
    ...
    public void removePlayer(EntityPlayerMP player)
    {
        if (this.players.contains(player))
        {
            ...
            if (this.chunk == null)
            {
                ...
                if (this.players.isEmpty())
                {
                    if (this.loading) net.minecraftforge.common.chunkio.ChunkIOExecutor.dropQueuedChunkLoad(this.playerChunkMap.getWorld(), this.pos.x, this.pos.z, this.loadedRunnable);
    ...
    public boolean providePlayerChunk(boolean canGenerate)
    {
        ...
        else
        {
            if (canGenerate)
            {
                this.chunk = this.playerChunkMap.getWorld().getChunkProvider().provideChunk(this.pos.x, this.pos.z);
            }
            else
            {
                this.chunk = this.playerChunkMap.getWorld().getChunkProvider().func_186028_c(this.pos.x, this.pos.z);
            }
        ...
        if (this.sentToPlayers && this.chunk != null)
        {
            if (this.changes != 0)
            {
                if (this.changes == 1)
                {
                    ...
                    net.minecraft.block.state.IBlockState state = this.playerChunkMap.getWorld().getBlockState(blockpos);
                ...
                //{// Forge: Send only the tile entities that are updated, Adding this brace lets us keep the indent and the patch small
                    for (int l = 0; l < this.changes; ++l)
                    {
                        ...
                        net.minecraft.block.state.IBlockState state = this.playerChunkMap.getWorld().getBlockState(blockpos1);
