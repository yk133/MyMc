...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void baseTick()
    {
        ...
        if (this.isAlive())
        {
            ...
            else
            {
                if (!this.canBreatheUnderwater() && !this.isPotionActive(MobEffects.WATER_BREATHING) && !flag1)
                {
                    ...
                    if (this.getAir() == -20)
                    {
                        ...
                        for (int i = 0; i < 8; ++i)
                        {
                            ...
                            this.world.func_175688_a(EnumParticleTypes.WATER_BUBBLE, this.posX + (double)f2, this.posY + (double)f, this.posZ + (double)f1, this.motionX, this.motionY, this.motionZ);
                        }
                ...
                if (!this.world.isRemote && this.isPassenger() && this.getRidingEntity() != null && this.getRidingEntity().shouldDismountInWater(this))
                {
                    this.stopRiding();
                }
            }
    ...
    protected void onDeathUpdate()
    {
        ...
        if (this.deathTime == 20)
        {
            ...
            for (int k = 0; k < 20; ++k)
            {
                ...
                this.world.func_175688_a(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.posY + (double)(this.rand.nextFloat() * this.height), this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d2, d0, d1);
            }
        }
    }
    ...
    protected void updatePotionEffects()
    {
        ...
        if (i > 0)
        {
            ...
            if (flag && i > 0)
            {
                ...
                this.world.func_175688_a(flag1 ? EnumParticleTypes.SPELL_MOB_AMBIENT : EnumParticleTypes.SPELL_MOB, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, d0, d1, d2);
            }
        }
    }
    ...
    public void renderBrokenItemStack(ItemStack stack)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            if (this.world instanceof WorldServer) //Forge: Fix MC-2518 spawnParticle is nooped on server, need to use server specific variant
                ((WorldServer)this.world).func_175739_a(EnumParticleTypes.ITEM_CRACK, vec3d1.x, vec3d1.y, vec3d1.z, 0,  vec3d.x, vec3d.y + 0.05D, vec3d.z, 0.0D, Item.getIdFromItem(stack.getItem()), stack.func_77960_j());
            else //Fix the fact that spawning ItemCrack uses TWO arguments.
                this.world.func_175688_a(EnumParticleTypes.ITEM_CRACK, vec3d1.x, vec3d1.y, vec3d1.z, vec3d.x, vec3d.y + 0.05D, vec3d.z, Item.getIdFromItem(stack.getItem()), stack.func_77960_j());
        ...
        if (!(entityIn instanceof EntityBoat) && !(entityIn instanceof AbstractHorse))
        {
            ...
            if (enumfacing1 != null)
            {
                ...
                for (int[] aint : aint1)
                {
                    ...
                    else if (!this.world.func_184143_b(axisalignedbb1.offset(0.0D, 1.0D, 0.0D)) && this.world.getBlockState(new BlockPos(d11, this.posY + 1.0D, d12)).isSideSolid(world, new BlockPos(d11, this.posY + 1.0D, d12), EnumFacing.UP))
                    {
                        d1 = d11;
                        d13 = this.posY + 2.0D;
                        d14 = d12;
    ...
    {
        this.motionY += 0.03999999910593033D * this.getAttribute(SWIM_SPEED).getValue();
    }
            ...
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).abilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).abilities.isFlying)
                {
                    ...
                    else
                    {
                        ...
                        if (this.onGround)
                        {
                            IBlockState underState = this.world.getBlockState(blockpos$pooledmutableblockpos.setPos(this.posX, this.getBoundingBox().minY - 1.0D, this.posZ));
                            f6 = underState.getBlock().getSlipperiness(underState, this.world, blockpos$pooledmutableblockpos, this) * 0.91F;
                        }
    ...
    public <T> T getCapability(net.minecraftforge.common.capabilities.Capability<T> capability, @Nullable net.minecraft.util.EnumFacing facing)
    {
        if (capability == net.minecraftforge.items.CapabilityItemHandler.ITEM_HANDLER_CAPABILITY)
        {
            ...
            else if (facing.getAxis().func_176720_b()) return (T) handHandler;
    ...
    public void moveRelative(float strafe, float up, float forward, float friction)
    {
        ...
        if (f >= 1.0E-4F)
        {
            ...
            {
                strafe = strafe * (float)this.getAttribute(SWIM_SPEED).getValue();
                up = up * (float)this.getAttribute(SWIM_SPEED).getValue();
                forward = forward * (float)this.getAttribute(SWIM_SPEED).getValue();
            }
