    ...
    public void livingTick()
    {
        if (this.world.isRemote)
        {
            ...
            if (!this.isInWater())
            {
                ...
                this.clientSideTouchedGround = this.motionY < 0.0D && this.world.func_175677_d((new BlockPos(this)).down(), false);
            }
            ...
            if (this.isMoving() && this.isInWater())
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    this.world.func_175688_a(EnumParticleTypes.WATER_BUBBLE, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width - vec3d.x * 1.5D, this.posY + this.rand.nextDouble() * (double)this.height - vec3d.y * 1.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width - vec3d.z * 1.5D, 0.0D, 0.0D, 0.0D);
                }
            }
