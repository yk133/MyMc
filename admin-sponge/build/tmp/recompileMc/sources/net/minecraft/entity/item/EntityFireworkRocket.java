...
public class EntityFireworkRocket extends Entity
{
    ...
    public void tick()
    {
        ...
        {
            this.world.func_175688_a(EnumParticleTypes.FIREWORKS_SPARK, this.posX, this.posY - 0.3D, this.posZ, this.rand.nextGaussian() * 0.05D, -this.motionY * 0.5D, this.rand.nextGaussian() * 0.05D);
        }
        ...
        if (f > 0.0F)
        {
            ...
            for (EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.getBoundingBox().grow(5.0D)))
            {
                if (entitylivingbase != this.boostedEntity && this.getDistanceSq(entitylivingbase) <= 25.0D)
                {
                    ...
                    for (int i = 0; i < 2; ++i)
                    {
                        RayTraceResult raytraceresult = this.world.func_147447_a(vec3d, new Vec3d(entitylivingbase.posX, entitylivingbase.posY + (double)entitylivingbase.height * 0.5D * (double)i, entitylivingbase.posZ), false, true, false);
    ...
    public void writeAdditional(NBTTagCompound compound)
    {
        ...
        {
            compound.put("FireworksItem", itemstack.write(new NBTTagCompound()));
        }
    }
