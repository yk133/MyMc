...
class ChunkIOProvider implements Runnable
{
    ...
    public void syncCallback()
    {
        ...
        // Load Entities
        this.loader.loadEntities(this.chunkInfo.world, this.nbt.getCompound("Level"), this.chunk);

        MinecraftForge.EVENT_BUS.post(new ChunkDataEvent.Load(this.chunk, this.nbt)); // Don't call ChunkDataEvent.Load async

        this.chunk.setLastSaveTime(provider.world.getGameTime());
        this.provider.chunkGenerator.func_180514_a(this.chunk, this.chunkInfo.x, this.chunkInfo.z);

        provider.loadedChunks.put(ChunkPos.asLong(this.chunkInfo.x, this.chunkInfo.z), this.chunk);
        this.chunk.onLoad();
        this.chunk.func_186030_a(provider, provider.chunkGenerator);
