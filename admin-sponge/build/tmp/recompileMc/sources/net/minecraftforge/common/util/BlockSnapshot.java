...
public class BlockSnapshot
{
    ...
    {
        this.setWorld(world);
        this.dimId = world.dimension.getDimension();
        this.pos = pos.toImmutable();
        this.setReplacedBlock(state);
        this.registryName = state.getBlock().getRegistryName();
        this.meta = state.getBlock().func_176201_c(state);
        this.setFlag(3);
        this.nbt = nbt;
        if (DEBUG)
        {
    ...
    public boolean restoreToLocation(World world, BlockPos pos, boolean force, boolean notifyNeighbors)
    {
        ...
        if (current.getBlock() != replaced.getBlock() || current.getBlock().func_176201_c(current) != replaced.getBlock().func_176201_c(replaced))
        {
            if (force)
            {
        ...
        if (DEBUG)
        {
            System.out.printf("Restored BlockSnapshot with data [World: %s ][Location: %d,%d,%d ][Meta: %d ][Block: %s ][TileEntity: %s ][force: %s ][notifyNeighbors: %s]", world.getWorldInfo().getWorldName(), pos.getX(), pos.getY(), pos.getZ(), replaced.getBlock().func_176201_c(replaced), replaced.getBlock().delegate.name(), te, force, notifyNeighbors);
        }
        return true;
    }
