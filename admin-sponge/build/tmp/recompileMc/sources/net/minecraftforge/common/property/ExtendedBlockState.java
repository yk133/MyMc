...
public class ExtendedBlockState extends BlockStateContainer
{
    ...
    protected static class ExtendedStateImplementation extends StateImplementation implements IExtendedBlockState
    {
        ...
        public <T extends Comparable<T>, V extends T> IBlockState func_177226_a(@Nonnull IProperty<T> property, @Nonnull V value)
        {
            ...
            return new ExtendedStateImplementation(getBlock(), clean.func_177228_b(), unlistedProperties, ((StateImplementation)clean).getPropertyValueTable(), this.cleanState);
        }
        ...
        public <V> IExtendedBlockState withProperty(IUnlistedProperty<V> property, @Nullable V value)
        {
            ...
            if (oldValue == null)
            {
                throw new IllegalArgumentException("Cannot set unlisted property " + property + " as it does not exist in " + getBlock().getStateContainer());
            }
            ...
            {
                throw new IllegalArgumentException("Cannot set unlisted property " + property + " to " + value + " on block " + Block.REGISTRY.getKey(getBlock()) + ", it is not an allowed value");
            }
            boolean clean = true;
            ImmutableMap.Builder<IUnlistedProperty<?>, Optional<?>> builder = ImmutableMap.builder();
            ...
            return new ExtendedStateImplementation(getBlock(), func_177228_b(), builder.build(), field_177238_c, this.cleanState);
        }
        ...
        public <V> V getValue(IUnlistedProperty<V> property)
        {
            ...
            if (value == null)
            {
                throw new IllegalArgumentException("Cannot get unlisted property " + property + " as it does not exist in " + getBlock().getStateContainer());
            }
