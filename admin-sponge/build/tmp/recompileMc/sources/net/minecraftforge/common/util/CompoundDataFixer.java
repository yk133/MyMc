...
public class CompoundDataFixer extends DataFixer
{
    ...
    private Map<String, Integer> getVersions(NBTTagCompound nbt)
    {
        ...
        ret.put("minecraft", nbt.contains("DataVersion", 99) ? nbt.getInt("DataVersion") : -1);
        if (nbt.contains("ForgeDataVersion", 10))
        {
            NBTTagCompound sub = nbt.getCompound("ForgeDataVersion");
            for (String key : sub.keySet())
            {
                ret.put(key, sub.contains(key, 99) ? sub.getInt(key) : -1);
            }
        }
        return ret;
    }
    ...
    public void writeVersionData(NBTTagCompound nbt)
    {
        ...
        nbt.put("ForgeDataVersion", sub);
        for (ModFixs mod : modFixers.values())
            sub.putInt(mod.mod, mod.version);
    }
}