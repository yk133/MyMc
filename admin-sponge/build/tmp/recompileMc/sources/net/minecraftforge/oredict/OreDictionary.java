...
public class OreDictionary
{
    ...
    private static void initVanillaEntries()
    {
        if (!hasInit)
        {
            ...
            registerOre("blockPrismarine", new ItemStack(Blocks.PRISMARINE, 1, BlockPrismarine.EnumType.ROUGH.func_176807_a()));
            registerOre("blockPrismarineBrick", new ItemStack(Blocks.PRISMARINE, 1, BlockPrismarine.EnumType.BRICKS.func_176807_a()));
            registerOre("blockPrismarineDark", new ItemStack(Blocks.PRISMARINE, 1, BlockPrismarine.EnumType.DARK.func_176807_a()));
    ...
    public static int[] getOreIDs(@Nonnull ItemStack stack)
    {
        ...
        {
            id = Item.REGISTRY.getId(stack.getItem().delegate.get());
        }
        List<Integer> ids = stackToId.get(id);
        if (ids != null) set.addAll(ids);
        ids = stackToId.get(id | ((stack.getDamage() + 1) << 16));
        if (ids != null) set.addAll(ids);
    ...
    public static boolean itemMatches(@Nonnull ItemStack target, @Nonnull ItemStack input, boolean strict)
    {
        ...
        return (target.getItem() == input.getItem() && ((target.func_77960_j() == WILDCARD_VALUE && !strict) || target.func_77960_j() == input.func_77960_j()));
    }
    ...
    private static void registerOreImpl(String name, @Nonnull ItemStack ore)
    {
        ...
        {
            hash = Item.REGISTRY.getId(ore.getItem().delegate.get());
        }
        if (ore.getDamage() != WILDCARD_VALUE)
        {
            hash |= ((ore.getDamage() + 1) << 16); // +1 so 0 is significant
        }
    ...
    public static void rebakeMap()
    {
        ...
        for (int id = 0; id < idToStack.size(); id++)
        {
            ...
            for (ItemStack ore : ores)
            {
                ...
                if (ore.getDamage() != WILDCARD_VALUE)
                {
                    hash |= ((ore.getDamage() + 1) << 16); // +1 so meta 0 is significant
                }
