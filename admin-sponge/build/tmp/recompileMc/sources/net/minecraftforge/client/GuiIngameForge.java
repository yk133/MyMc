...
public class GuiIngameForge extends GuiIngame
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        mc.entityRenderer.func_78478_c();
        GlStateManager.enableBlend();

        if (renderVignette && Minecraft.isFancyGraphicsEnabled())
        {
            func_180480_a(mc.player.getBrightness(), res);
        }
        else
        {
        ...
        ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(mc.player.func_70005_c_());
    ...
    protected void renderSubtitles(ScaledResolution resolution)
    {
        ...
        this.overlaySubtitle.func_184068_a(res);
        post(SUBTITLES);
    }
    ...
    protected void func_180479_a(ScaledResolution res, float partialTicks)
    {
        ...
        {
            this.spectatorGui.func_175264_a(res, partialTicks);
        }
        else
        {
    ...
    protected void renderAir(int width, int height)
    {
        ...
        if (player.func_70055_a(Material.WATER))
        {
        ...
        if (this.mc.gameSettings.heldItemTooltips && !this.mc.playerController.func_78747_a())
        {
            ...
            if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty())
            {
                ...
                if (opacity > 0)
                {
                    ...
                    if (font != null)
                    {
                        int x = (res.func_78326_a() - font.getStringWidth(name)) / 2;
                        font.drawStringWithShadow(name, x, y, WHITE | (opacity << 24));
                    }
                    else
                    {
                        int x = (res.func_78326_a() - fontrenderer.getStringWidth(name)) / 2;
    ...
    protected void renderRecordOverlay(int width, int height, float partialTicks)
    {
        if (overlayMessageTime > 0)
        {
            ...
            if (opacity > 0)
            {
                ...
                fontrenderer.func_78276_b(overlayMessage, -fontrenderer.getStringWidth(overlayMessage) / 2, -4, color | (opacity << 24));
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
    ...
    protected void renderTitle(int width, int height, float partialTicks)
    {
        if (titlesTimer > 0)
        {
            ...
            if (opacity > 8)
            {
                ...
                this.getFontRenderer().func_175065_a(this.displayedTitle, (float)(-this.getFontRenderer().getStringWidth(this.displayedTitle) / 2), -10.0F, 16777215 | l, true);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
                GlStateManager.scalef(2.0F, 2.0F, 2.0F);
                this.getFontRenderer().func_175065_a(this.displayedSubTitle, (float)(-this.getFontRenderer().getStringWidth(this.displayedSubTitle) / 2), 5.0F, 16777215 | l, true);
                GlStateManager.popMatrix();
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
