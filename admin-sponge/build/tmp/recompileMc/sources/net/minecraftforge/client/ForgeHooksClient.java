...
public class ForgeHooksClient
{
    ...
    public static void preDraw(EnumUsage attrType, VertexFormat format, int element, int stride, ByteBuffer buffer)
    {
        ...
        {
            case POSITION:
                GlStateManager.vertexPointer(count, constant, stride, buffer);
            ...
            case NORMAL:
                ...
                GlStateManager.normalPointer(constant, stride, buffer);
                GlStateManager.enableClientState(GL11.GL_NORMAL_ARRAY);
                break;
            case COLOR:
                GlStateManager.colorPointer(count, constant, stride, buffer);
                GlStateManager.enableClientState(GL11.GL_COLOR_ARRAY);
                break;
            case UV:
                OpenGlHelper.glClientActiveTexture(OpenGlHelper.GL_TEXTURE0 + attr.getIndex());
                GlStateManager.texCoordPointer(count, constant, stride, buffer);
    ...
    public static IBakedModel getDamageModel(IBakedModel ibakedmodel, TextureAtlasSprite texture, IBlockState state, IBlockAccess world, BlockPos pos)
    {
        ...
        return (new SimpleBakedModel.Builder(state, ibakedmodel, texture, pos)).build();
    }
